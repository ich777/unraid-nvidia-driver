# Clone and compile Nvidia installer
if [ ! -d /tmp/nvidia-installer ]; then
  cd /tmp
  git clone https://github.com/NVIDIA/nvidia-installer
  cd /tmp/nvidia-installer
  # Patch: https://patch-diff.githubusercontent.com/raw/NVIDIA/nvidia-installer/pull/13.patch
  cp ${DATA_DIR}/nvidia.patch /tmp/nvidia-installer
...
# Make the Nvidia driver executable and decompress it in a temporary directory
chmod +x ${DATA_DIR}/NVIDIA_v${NV_DRV_V}.run
${DATA_DIR}/NVIDIA_v${NV_DRV_V}.run --extract-only
cd ${DATA_DIR}/NVIDIA-*${NV_DRV_V}

# Copy over patched installer
cp $(find /tmp/nvidia-installer -type f -name "nvidia-installer") .
...
${DATA_DIR}/NVIDIA*${NV_DRV_V}/nvidia-installer --kernel-name=$UNAME \
